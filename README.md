...
Original task description is attached after.

Bootstrapped using [typescript-starter](https://github.com/bitjson/typescript-starter)

# Running

Clone and install packages. The project expose `robotMain` function that you can pass array of strings (commands) and it will do the rest.
`main.spec.ts` have a test that generate 10000 command array and run it, all errors and outputs are logged in the console.
you can run that using:
```sh
npx ava --match="robotMain"
```
...
# Original task description (Robot Challenge):
